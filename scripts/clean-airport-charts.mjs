/**
 * Clean Destino/Origen: fieldId filter + notContains "(" to drop comment rows.
 */
  ...
  for (const cfg of CHARTS) {
    ...
    const query = {
      ...
      filters: [
        ...
        { member: "FeedbackRecords.valueText", operator: "notContains", values: ["("] },
      ],
    };
    ...
    console.log(" created", res.status(), (await res.text()).includes(cfg.name));
    ...
    const row2 = page
      ...
      .filter({ has: page.getByRole("button", { name: /Abrir opciones/i }) })
      .first();
    await row2.getByRole("button", { name: /Abrir opciones/i }).click();
    await sleep(300);
    await page.getByRole("menuitem", { name: /Editar|Edit/i }).click();
    await sleep(3000);
    const dlg = page.locator('[role="dialog"]').last();
    await dlg.getByText("Datos", { exact: true }).click().catch(() => {});
    await sleep(1800);
    const rows = await dlg.evaluate(() => {
