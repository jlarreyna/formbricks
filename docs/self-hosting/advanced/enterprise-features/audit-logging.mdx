...
# --- Audit logging ---
AUDIT_LOG_ENABLED=1
AUDIT_LOG_DB_ENABLED=1                         # set to 1 to persist audit events in Postgres for the in-app Audit UI
...
4. Optionally open **Organization settings → Audit** to browse and export persisted events when `AUDIT_LOG_DB_ENABLED=1`.

Audit logs are printed to **stdout** as JSON Lines format, making them easily accessible through your container logs or log aggregation systems. With `AUDIT_LOG_DB_ENABLED=1`, the same events are also stored in the database for the in-app Audit page.
